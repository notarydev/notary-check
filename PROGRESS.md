...
**Last updated**: 2026-09-04 — **Locked case 2 is fixed and live.** The paraphrased-contradiction failure open since 2026-09-02 ("declined 12 percent" vs "grew 17%" returning UNSUPPORTED) is resolved and verified against the live deployment. The documented hypothesis was wrong: the judge read "declined" as a decrease perfectly, 3/3. The real cause was entity comparison — claim `Acme` vs passage `Acme Corp`, where `normalizeEntity` canonicalised suffix *spelling* but not *presence*, so entity mismatched, the candidate was ruled inapplicable, and it was dropped before the state machine saw it. Fixed with a deliberately asymmetric optional-suffix rule ("Acme Corp" vs "Acme Inc" still mismatches; locked case 6 untouched). Also shipped: the claim loop is now bounded-concurrent (execution fans out, accumulation stays in claim order so the first-come caps stay deterministic), and the `not_checked` card state is live so an unsourced claim no longer reports as a Notary malfunction. Live images `:notary-check-api.engine.16` and `:notary-check-mcp.server.17`. Engine 356/356 against real Postgres, server 6/6. Earlier the same night: migrations `0014` (Advance feature flag) and `0015` (millicent cost metering — every one of 228 historical usage rows had metered as **0 cents**, so both spend caps had never had anything to sum) were applied to production, and Advance's required adversarial evaluation was run for the first time (21 case-runs, 0 violations). One self-inflicted outage along the way, recorded in `docs/build/architecture-and-progress.md`: migrations were applied before the matching image was deployed, and because `insertUsageEvent` is awaited bare inside `reviewFlow`, every judge-invoking review failed until the deploy landed. Correct order is deploy-then-migrate.

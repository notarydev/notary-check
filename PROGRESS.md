...
**Fully shipped 2026-09-05**: `:notary-check-api.engine.42` / `:notary-check-mcp.server.40` (deployments 18 and 24), including migrations through `0017`. E-LAT (entity fail-fast + parallel residual fields) and E-MEAS (findings and gaps persisted) are both live; E-MEAS verified by reading production rows, E-LAT verified only as non-regressive — the saving needs a real multi-claim answer to measure, see `ROADMAP.md`. Nothing on `main` is undeployed. Move telemetry is live — the invocation path now persists (`act_invocation` rows with no claim_id went 0 → 2) and `POST /v1/move-events` is routed and auth-gated. `act_move_event` fills on the first real conversation, not on a smoke test.

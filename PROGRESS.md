...
- **Track 2 cut loose from the claim loop.** It rode on the per-claim path and the connector returned early with no claims, so Advance never ran on the majority of real turns — silent exactly where it was the only useful output.
- **Intent inference** (`advance/intent.ts`), deterministic and ours rather than Claude's. An unmatched request returns `general`, which keeps the *full* move set — abstaining never narrows anything.
- **A structured handoff.** Track 2 was given one sentence and had to guess whether it was looking at a wrong period, a wrong entity, or a wrong number. It now receives the field deltas, which were already computed and discarded at that boundary. Still sealed: no evidence corpus, no rejected-candidate pool, and a test asserts the passage never crosses.
...
**Five bugs found after "shipping", all invisible to a passing test suite.** Recorded in full in `docs/build/architecture-and-progress.md`. In short: extraction was truncating on 20-40% of real answers; self-contradiction was unreachable because the extractor and the detector each did something correct that cancelled the other; the card discarded Track 2's suggestions on exactly the states where Track 2 was the only thing running; an instruction in the tool result was correctly refused by Claude as prompt injection; and Advance ran six times per review on the old per-claim path, so the intent inference and structured handoff shipped and never executed. The suite passed 388-391 throughout. Each became visible the moment real data met the real code path — and the check that caught the last one was reading production rows, not looking at the card.

**Not built, so it is not mistaken for done:** Track 2 has **no detectors** — it is one model call with four moves, and intent is used only to pick which are legal. Findings are **not persisted**, so nothing measures which detectors fire in production. The **card cannot render** the new output (a claim with no source *and* a self-contradiction has no representation). There is **no ask ledger**, so a gap can repeat on every invocation, and **no `responds_to`**, so we cannot tell whether asking ever led to a repair.

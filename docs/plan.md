...
Use structured extraction on `answer_text`. Exclude greetings, creative writing, uncheckable opinion, and transitions. For each candidate, recover entity, period, measure, value/unit, comparator/baseline, modality, nearby source associations, and materiality. This is a checkability decision, not a truth score.
...
1. **The claim is decomposed into its applicability fields** — this decomposition already exists (entity, time, scope, measure, value/unit, baseline, modality); it doesn't need to be invented.
...
**What the judge outputs instead, per field, per the decomposition already fixed above (entity, time, scope, measure, value/unit, baseline, modality) — using the four-outcome vocabulary fixed in § Judge authority boundary, above:**
...
| Cost | Per-review budget and organization monthly hard cap enforced. |

...
| **Per-conversation standing consent** | The user names Notary once ("use Notary for anything I ask you to check in this conversation"); Claude then keeps invoking it for the rest of that same conversation without being asked again each turn — reported directly by Claude itself, 2026-09-03, describing how it handles third-party connector tools generally: invocation needs an explicit trigger, but that trigger can be a standing instruction that persists for the session, not a fresh ask every time. | **Not yet independently confirmed with a live multi-turn test** (say it once, then send several more claim-check-worthy messages without naming Notary again, verify it keeps firing) — treat as a real, plausible, actionable mechanism, not yet a proven one. **Does not carry across conversations** — a new chat needs the phrase again. If confirmed, this is a genuine, buildable onboarding win: teach users the exact phrase once, not "hope the model calls it." |
...
Prompting is a dial worth turning for an enterprise deployment that controls its own system prompt — it is not a substitute for WATCH, and no amount of prompt engineering should ever be described internally or externally as closing this gap completely. Per-conversation standing consent, once confirmed, is a real product lever worth designing onboarding copy around ("say this once per chat") — but it's a *usability* improvement, not a coverage guarantee, and must never be described as one either.
